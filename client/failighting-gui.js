    ...
    for (var i=0; i<config.devices.length; i++) {
      ...
      devices[i].handlers = {
        ...
        updateValuesHandler: (device) => {
          ...
          //var valuedisplay = document.getElementById(device.name+"_deviceinfo");
          //valuedisplay.textContent = device.name+" - ["+device.values.concat()+"]";
        },
...
const addDevicetoDOM = (device) => {
    ...
    let info = devicediv.appendChild(document.createElement("div"));
    info.textContent = device.name;
    info.classList.add("deviceinfo");
    info.id = device.name+"_deviceinfo";
