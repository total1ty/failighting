...
class failScene {
  ...
  trigger() {
    for (let i=0; i<this.devicevalues.length; i++) {
      if(this.scenedevices[i].values.length == this.devicevalues[i].values.length) {
        this.scenedevices[i].values = this.devicevalues[i].values;
        this.scenedevices[i].sendValues();
      }
      else {
